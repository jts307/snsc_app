...
class EditResourceFields extends StatefulWidget {
  const EditResourceFields({
    Key? key,
    required this.resource,
  }) : super(key: key);

  final Organization resource;

  @override
  State<EditResourceFields> createState() => _EditResourceFieldsState();
}

class _EditResourceFieldsState extends State<EditResourceFields> {
  Uint8List? imageBytes;
  late bool initializeFilters;
  ...
  updateOrganization() async {
    ...
    if (!noInternet) {
      ...
      if (imageBytes != null) {
        ...
        try {
          var request =
              http.MultipartRequest('POST', Uri.parse(ApiConfig.herokuUploads));
        ...
        await APIService.updateOrganization(widget.resource.id!, newOrg,
            user.token!, widget.resource.imageURL!, context);
      } else {
        Organization newOrg = Organization(
            name: name.text,
            descriptions: description.text,
  ...
  List<Filter> getFilterLFromStringL(List<Filter> fl, List<String> sl) {
    final matchingSet = HashSet.from(sl);
    return fl.where((item) => matchingSet.contains(item.name)).toList();
  }

  String getSavedAge(Age ageObject) {
    if (ageObject.allAges == true) {
      return "ALL";
    } else {
      return "${ageObject.lowerRange}-${ageObject.upperRange}";
    }
  }

  String convertFee(String fee) {
    if (fee == "true") {
      return "YES";
    } else {
      return "NO";
  ...
  Future<void> getOrgImageBytes() async {
    Map<String, String> requestHeaders = {
      'Content-Type': 'application/json',
    };
    if (kDebugMode) {
      print(widget.resource.imageURL);
    }
    if (widget.resource.imageURL != "None") {
      var url = Uri.https(
          ApiConfig.rootURL, '/api/images/${widget.resource.imageURL}');
    ...
    //     await rootBundle.load('Assets/stock_resource_image.jpeg');
  ...
  void initState() {
    ...
    disabilitesChosen = widget.resource.disabilitiesServed!.cast();
    servicesChosen = widget.resource.servicesProvided!.cast();
    statesChosen = widget.resource.statesServed!.cast();
    insurancesChosen = widget.resource.insurancesAccepted!.cast();
    ...
    fee = convertFee(widget.resource.fee!);

    prevFee = convertFee(widget.resource.fee!);

    //check for null errors
    name.text = widget.resource.name!;
    description.text = widget.resource.descriptions!;
    phone.text = widget.resource.primaryPhoneNumber!;
    email.text = widget.resource.primaryEmail!;
    allPhone.text = widget.resource.fullPhoneNumber!;
    allEmail.text = widget.resource.fullEmail!;
    website.text = widget.resource.primaryWebsite!;
    allWebsite.text = widget.resource.fullWebsite!;
    contactPersonName.text = widget.resource.primaryContactName!;
    contactPersonRole.text = widget.resource.primaryContactRole!;
    ageController.text = getSavedAge(widget.resource.agesServed!);
    feeDescriptionController.text = widget.resource.feeDescription!;
        ...
        if (snapshot.connectionState == ConnectionState.none) {
          ...
          if (snapshot.hasData) {
            if (initializeFilters) {
              previousDisChosen = getFilterLFromStringL(
                  snapshot.data[0], widget.resource.disabilitiesServed!.cast());
              previousServChosen = getFilterLFromStringL(
                  snapshot.data[1], widget.resource.servicesProvided!.cast());
              previousStateChosen = getFilterLFromStringL(
                  snapshot.data[2], widget.resource.statesServed!.cast());
              previousInsurChosen = getFilterLFromStringL(
                  snapshot.data[3], widget.resource.insurancesAccepted!.cast());
                                ...
                                showDialog(
                                    ...
                                    builder: (BuildContext context) =>
                                        OrgInputField(
                                          ...
                                          labelText: "Primary Email Addresses",
                                          minLines: 1,
